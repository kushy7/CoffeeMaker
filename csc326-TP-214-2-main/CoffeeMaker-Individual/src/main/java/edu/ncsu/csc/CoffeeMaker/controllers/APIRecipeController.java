...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity updateRecipe ( @RequestBody final Recipe recipe ) {
        Recipe model = service.findByName( recipe.getName() );
        if ( null == model ) {
            return new ResponseEntity( errorResponse( "Recipe with the name " + recipe.getName() + " does not exist" ),
                    HttpStatus.NOT_FOUND );
        }
        model.setPrice( recipe.getPrice() );
        for(Ingredient i : model.getIngredients()) {
